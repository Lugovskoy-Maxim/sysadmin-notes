...
export default function PricingPage() {
  ...
  const [localBilling, setLocalBilling] = useState<BillingStatus | null>(billing);
  ...
  async function choosePlan(planId: PlanId) {
    if (planId === "free") {
      window.location.href = token ? "/" : "/?register=1";
      return;
    }
    if (!token) {
  ...
  return (
    <main className="pricing-page">
      ...
      <section className="pricing-hero">
        ...
        <h1>Работайте вместе над инфраструктурой</h1>
        <p>
          Бесплатный тариф — для личных заметок. Pro и Team открывают склад, оснащение, карту сети
          и совместное редактирование проектов.
        </p>
        {localBilling ? (
